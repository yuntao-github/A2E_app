...
output_folder2 = strcat(currentFolder,filesep,"000_output4code_version_no_rounding20231121");
addpath(strcat(codeFolder2,filesep,"othercolor",filesep,"othercolor"))

%%
outputfoldername_all = ["error_type0";"error_type0";"error_type0"];
age_symboles = {'d', 'p', 's', 'o','^','<','>','v','*','+'};
age_types = {'AHe','AFT','ZHe','ZFT','THe','TFT','KAr','BAr','MAr','HAr'};
the3transects=["Fitzgerald1995","DhauladarRange","K1TB"];
the3transectsNames=["Denali","Dhauladar Range","KTB (0-1 km)"];
ylim_value = [1.0, 1.8, .3];
abc=["a", "b", "c"];
...
for iii=1:length(the3transects)
    ...
    set(gca,'FontSize',8)
    title(strcat("(",abc(iii),") ", the3transectsNames(iii)),'FontSize',10);
...
%     scatter(Age, zeros(length(Age),1))
    ...
    text(.2, .8, {text2print1;text2print2;text2print4},'fontsize', 7,'Units','normalized');
...
%     set(gca,'xdir','reverse')
    ...
    yyaxis right
    hold on
    fill([0 tmax tmax 0],[G_present-G_present_error,G_present-G_present_error, G_present+G_present_error,G_present+G_present_error],'c','EdgeColor','none','FaceAlpha',.1)
...
cd(codeFolder2)
% set(gcf,'PaperSize',[13 20]); %set the paper size to what you want  
set(gcf,'Position', [50 50 560 810]);
% saveas(gcf, 'plots_figures_4the3transects.pdf')
print(gcf,'plots_figures_4the3transects','-dpdf') % then print it
