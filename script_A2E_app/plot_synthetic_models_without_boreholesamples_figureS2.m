...
output_folder2 = strcat(currentFolder,filesep,"outputs_synthetic_models",filesep,"without_boreholesamples");
...
%     output_folder2 = '/Users/yuntao/Documents/MACandPC/software/modeling_vertical_profiles/synthetic_model_without_boreholes';
% end

outputfoldername_all = ["error_type0";"error_type0";"error_type0";"error_type0";"error_type0";"error_type0"];
...
ylim_value = [1.5, 1.5, 1.5, 1.5, 1., 1.];
abc=["a", "b", "c","d", "e","f"];
figure('Renderer', 'painters', 'Position', [50 50 750 1100])
n_cols_subplot = length(the3transects);
...
%     scatter(Age, zeros(length(Age),1))
    ...
    text(.13, .83, {text2print1;text2print2;text2print4},'fontsize', 7,'Units','normalized','Interpreter','tex');
...
cd(codeFolder2)
% set(gcf,'PaperSize',[20 20]); %set the paper size to what you want  
set(gcf,'Position', [50 50 700 770]);
% saveas(gcf, 'plots_figures_4the3transects.pdf')
print(gcf,'plots_figures_synthetic_transects_without_boreholes','-dpdf') % then print it
