...
%% for testing the effect of deltat
datafolder='000';
...
dt2plot_authority = [.5, 1.0, 1.5, 3.5, 4.5, 5.0]; % if dt2plot_authority is defined, it will be used in the following calculations
...
%     dt = (dz^2.)/kappa/0.02;
    ...
    %% test the effect of prior time interval length on the inversion results, it seems the effect is minor

    fprintf('Running inversion with different e0 \n');
    Tgrd = Tgrd_initial_authority;
    n_erun = length(dt2plot_authority);
...
%             e0=e0_pr*(1+(irun-n_erun/2)*0.2);   % range between +/-80%
        ...
        dt2plot = dt2plot_authority(irun);
        tmax = ceil(start_time_authority/dt2plot)*dt2plot;
        Nt = fix(tmax/dt2plot);  % number of steps of calculation 
        
        formatSpec = '  e0 = %4.2f +/- %4.2f, delta_t = %4.2f\n';
        fprintf(formatSpec,e0,sigma,dt2plot)
        cd (codeFolder)
        inversion_clc2;
        dirname=strcat('RUN_deltat', num2str(irun));
        cd (output_folder)
...
disp("plotting the results by running the code: xxx")
