...
e0_pr_authority = .5; % if e0_pr_authority is defined, it will be used in the following calculations
e0_pr_err_authority = .25; % if e0_pr_err_authority is defined, it will be used in the following calculations
Tgrd_initial_authority = [16, 20, 22, 26, 30, 34]; %  if Tgrd_initial_authority is defined, it will be used in the following calculations and the iterations for this value will not be run
...
% nt4each_step = 50; % this value (so as to make sure a dt <0.25) should be larger enough to yield stable results
plottingOrNot = 1;
...
%     dt = (dz^2.)/kappa/0.02;
    ...
    if exist('Tgrd_initial_authority', 'var')
        n_Grun = length(Tgrd_initial_authority);
    else
        n_Grun = 5;
    end
    if exist('e0_pr_authority', 'var')
...
%         % the following lines are for checking zc, h and p calculations
%         for irun = 1:n_Grun
%             Tgrd=30;
%             e0 = 0.17;  % assumed initial erosion rate [km/my]
%             sigma = 0.04;  % error of the assumed initial erosion rate [km/my]
%             fprintf('  G0 = %d\n', Tgrd);
%             inversion_clc2;
%             cd (datafolder_ii)
%             dirname=strcat('RUN_G', num2str(irun));
%             mkdir(dirname);
%             cd (dirname)
%             misfit1(irun,1)=misfit_age;
%             misfit_G=((dTdz-G_present)./G_present_error).^2;
%             misfit1(irun,2)=misfit_G;
%             clear plottingOrNot;
%             save run.mat
%             cd ..
%             cd ..
...
%     else
        G_fraction = linspace(0.5, 0.9, n_Grun);
        for irun = 1:n_Grun
            Tgrd=Tgrd_initial_authority(irun); % decrease by 10% each stage
            fprintf('  G0 = %d\n', Tgrd);
            cd (codeFolder)
            inversion_clc2;
            cd (output_folder)
...
%             sigma=e0_pr_err_authority(irun);
%         else
%             e0=e0_pr*e_fraction(irun);   % change between 0.2*e0_pr and 2*e0_pr
% %             e0=e0_pr*(1+(irun-n_erun/2)*0.2);   % range between +/-80%
%             sigma=e0*0.3;
%         end
%         
...
%         mkdir(dirname);
%         cd (dirname)
%         misfit2(irun,1)=misfit_age;
%         misfit2(irun,4)=e0;
%         misfit_G=((dTdz(end,1)-G_present)./G_present_error).^2;
%         misfit2(irun,2)=misfit_G;
