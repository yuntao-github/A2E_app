...
%% for testing the effect of sigma0
datafolder='000';
...
e0_pr_err_authority_frac = [0.1, 0.3, 0.4, 0.6, 0.8, 1.0]; % if e0_pr_err_authority is defined, it will be used in the following calculations
...
%         n_erun = length(e0_pr_err_authority);
%     else
%         n_erun = 10;
%         sigma = e0*0.3;  % error of the assumed initial erosion rate [km/my]
%     end
% 
...
%         for irun = 1:n_Grun
%             Tgrd=G_present*G_fraction(irun); % decrease by 10% each stage
%             fprintf('  G0 = %d\n', Tgrd);
%             cd (codeFolder)
%             inversion_clc2;
%             cd (datafolder_ii)
...
%     end
    ...
    fprintf('Running inversion with different sigma0 \n');
    Tgrd = Tgrd_initial_authority;
    n_erun = length(e0_pr_err_authority_frac);
    ...
    for irun = 1:n_erun
        e0 = e0_pr_authority;
        sigma=e0_pr_err_authority_frac(irun)*e0;
...
%         end
        ...
        cd (codeFolder)
        inversion_clc2;
        dirname=strcat('RUN_sigma', num2str(irun));
        cd (output_folder)
